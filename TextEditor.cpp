...
void TextEditor::Render()
{
    ...
    if(!mLines.empty())
    {
        ...
        while(lineNo <= lineMax)
        {
            ...
            {
                auto end =
                    ImVec2(lineStartScreenPos.x + contentSize.x + 2.0f * scrollX, lineStartScreenPos.y + mCharAdvance.y);
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::GCode()
{
    ...
    {
        static const char* const keywords[] = {
            "G0",  "G1",  "G2",  "G3",  "G4",   "G10",  "G17",  "G18",  "G19",  "G20",  "G21", "G28", "G30",
            "G38", "G40", "G41", "G42", "G53",  "G54",  "G55",  "G56",  "G57",  "G58",  "G59", "G80", "G81",
            "G82", "G83", "G84", "G85", "G86",  "G87",  "G88",  "G89",  "G90",  "G91",  "G92", "G93", "G94",

            "M0",  "M1",  "M2",  "M3",  "M4",   "M5",   "M6",   "M7",   "M8",   "M9",   "M30", "M48", "M49",
            "M60", "M82", "M83", "M84", "M104", "M105", "M106", "M107", "M109", "M140", "M190"};

        for(auto& k : keywords)
            langDef.mKeywords.insert(k);

        static const char* const identifiers[] = {"X", "Y", "Z", "A", "B", "C", "I", "J", "K",
                                                  "E", "F", "S", "P", "R", "Q", "T", "N"};

        for(auto& k : identifiers)
        {
            Identifier id;
            id.mDeclaration = "G-code parameter";
            langDef.mIdentifiers.insert(std::make_pair(std::string(k), id));
        }
        ...
        langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>("[gG][0-9]+(\\.[0-9]+)?", PaletteIndex::Keyword));

        langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>("[mM][0-9]+(\\.[0-9]+)?", PaletteIndex::Keyword));

        langDef.mTokenRegexStrings.push_back(std::make_pair<std::string, PaletteIndex>("[a-zA-Z]", PaletteIndex::Identifier));
